...
public abstract class ${output.currentClass}<E extends Identifiable<PK>, PK extends Serializable> {
    ...
    public ${output.currentClass}(GenericRepository<E, PK> repository, GenericPermission<E> permission, GenericPrinter<E> printer, String selectUri, String editUri) {
...
$output.require("com.jaxio.jpa.querybyexample.MetamodelUtil")##
    ...
    public boolean shouldReplace(E currentEntity, E selectedEntity) {
        ...
        if (currentEntity != null && selectedEntity != null && currentEntity.${identifiableProperty.iser}() && selectedEntity.${identifiableProperty.iser}()) {
            if (selectedEntity.${identifiableProperty.getter}().equals(currentEntity.${identifiableProperty.getter}())) {
    ...
    public ConversationContext<E> newEditContext(E e) {
        return new ConversationContext<E>().entity(e).isNewEntity(!e.${identifiableProperty.iser}()).viewUri(getEditUri());
    }
