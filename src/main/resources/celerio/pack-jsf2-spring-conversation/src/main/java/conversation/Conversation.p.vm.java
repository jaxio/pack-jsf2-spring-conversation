...
##
$output.java($WebConversation, "Conversation")##

$output.requireStatic($WebConversation, "ConversationUtil.cidParamValue")##
...
$output.require("java.util.concurrent.atomic.AtomicInteger")##
...
public class $output.currentClass implements Serializable {
    ...
    protected void handleOutOfSynchContext(String ccid, HttpServletRequest request) throws UnexpectedConversationException {
        ...
        if (requestedContext != null) {
            ...
            throw new UnexpectedConversationException("Uri not in sync with conversation expecting _ccid_="+
                    getCurrentContext().getId() + " but got " + ccid, request.getRequestURI(), getUrl());
        }
    }
